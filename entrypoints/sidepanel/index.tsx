...
function TabButton({ view, active, locale, icon, onChange }: { view: SidePanelView; active: SidePanelView; locale: Locale; icon: React.ReactNode; onChange: (view: SidePanelView) => void }) {
  ...
  return <button className={active === view ? "active" : ""} type="button" aria-current={active === view ? "page" : undefined} onClick={() => onChange(view)}>{icon}<span>{labels[view][locale === "zh" ? 0 : 1]}</span></button>;
}
